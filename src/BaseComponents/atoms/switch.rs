...
pub fn Switch(
    ...
    rsx! {
        button {
            class: tw_merge!(
                "group w-[80px] p-[10px] rounded-[10px] flex justify-start items-center bg-background ease-quick",
                class
            ),
...
pub fn FloatingSwitch(
    ...
    rsx! {
        div {
            ...
            div {
                class: tw_merge!(
                    "z-20 transition-all ease-quick duration-500 absolute inset-y-0 rounded-[30px] ",
                    floater
                ),
                left: "{left()}px",
                width: "{width()}px",
